...
export const get = async (uri: string, data: Record<string, string> = {}, onExpired: Function | null = null): Promise<any> => {
...
export const createSocket = (path: string, onOpen: Function, onMessage: any, onClose: any): WebSocket => {
...
const refreshToken = async (onExpired) => {
  ...
  if (body.code && body.code !== "0") {
    if (onExpired) {
      ...
      // todo 최상단 popup 및 App_backup.vue 에 emit 을 통해 첫화면으로 돌아가야 한다.
