...
import {AmbientLight, Matrix4, Mesh} from "three";
import GameCanvasDrawer from "@/scene/game/GameCanvasDrawer";
...
export default class GameCanvas implements Canvas {
  vue: GameScene
  ctx: AppManager | undefined
  drawer: GameCanvasDrawer | undefined
  loadingBackground: Mesh | undefined
  ...
  setKeyMapping() {
    ...
    for (const command in this.keySettings) {
      let func
      const keys = this.keySettings[command]
      if (command === "play1") {
        func = this.keyPlay1
